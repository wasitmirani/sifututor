<template>
    <div class="table-responsive text-nowrap">
        <table class="table">
            <caption class="ms-4">Creditor Payments
            </caption>
            <thead>
                <tr>
            ...
            <tbody>
                <tr v-for="(item, index) in desserts" :key="index">
                    <td> {{ item.uid ?? "N/A" }}</td>
                    <td> {{ item.creditor_name ?? "N/A" }}</td>
                    <td> {{ item.payment_amount ?? "N/A" }}</td>
                    <td> {{ item.floating_amount ?? "N/A" }}</td>
                    <td> {{ item.payment_date ?? "N/A" }}</td>
                    <td> {{ item.related_credior_invoice ?? "N/A" }}</td>
                    <td class="" style=""><div class="d-flex align-items-center">
                        <router-link :to="`/creditor/payments/edit/${item.slug}/${item.uid}`" class="text-body"><i class="ti ti-edit ti-sm me-2 text-primary"></i></router-link > |
